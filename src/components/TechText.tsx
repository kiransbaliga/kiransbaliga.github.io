...
export interface TechTextProps {
  ...
  speed?: number;
  scaleToFit?: boolean;
  onMeasure?: (metrics: { inkWidth: number; size: number }) => void;
  className?: string;
  style?: CSSProperties;
}
...
const TechText: React.FC<TechTextProps> = ({
  ...
  useEffect(() => {
    ...
    let glyphs: any[] = [];
    let presence = 0;
    let clock = settingsRef.current?.sweepOffset ?? sweepOffset ?? 0;
    let pulse = settingsRef.current?.sweepOffset ?? sweepOffset ?? 0;
    let placed = false;
    let dragging = -1;
    ...
    const ensureLayout = (s: any) => {
      ...
      const computedFs = parseFloat(getComputedStyle(container).fontSize);
      const baseSize = s.fontSize || (computedFs > 0 ? computedFs : 130);
      let size = baseSize;
      if (s.scaleToFit) {
        setFont(probe, s, baseSize);
        let m = probe.measureText(s.text);
        const availableWidth = Math.max(1, width - PADDING_X * 2);
        const fit = Math.min(
          1,
          availableWidth / Math.max(m.actualBoundingBoxLeft + m.actualBoundingBoxRight, 1)
        );
        size = baseSize * fit;
      }
      ...
      const x =
        s.align === 'left'
          ? PADDING_X + m.actualBoundingBoxLeft
          : s.align === 'right'
          ? width - PADDING_X - inkWidth + m.actualBoundingBoxLeft
          : (width - inkWidth) / 2 + m.actualBoundingBoxLeft;
      const baseline = PADDING_Y + (availableHeight - inkHeight) / 2 + m.actualBoundingBoxAscent;
      const next = {
        size,
        baseline,
