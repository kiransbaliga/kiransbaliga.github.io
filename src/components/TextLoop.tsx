...
const buildPath = (shape: string, curviness: number, ribbonWidth: number, vw: number, vh: number) => {
  const cx = vw / 2;
  const cy = vh / 2;
  ...
  switch (shape) {
    ...
    case 'arch': {
      ...
      return `M 120 ${cy + rise / 2} Q ${cx} ${cy - rise * 1.5} ${vw - 120} ${cy + rise / 2}`;
    }
    case 'line':
      return `M -160 ${cy} L ${vw + 160} ${cy}`;
    case 'wave':
    default: {
      const a = Math.min(c * 2.2, room * 2);
      return `M -320 ${cy} Q -160 ${cy - a} 0 ${cy} T 320 ${cy} T 640 ${cy} T 960 ${cy} T 1280 ${cy} T ${vw + 320} ${cy}`;
    }
  }
};
...
const TextLoop: React.FC<TextLoopProps> = ({
  ...
  const vw = viewWidth || (shape === 'line' ? 500 : DEFAULT_W);
  const vh = viewHeight || (shape === 'line' ? 32 : DEFAULT_H);
  ...
  const d = useMemo(() => path || buildPath(shape, curviness, ribbonWidth, vw, vh), [path, shape, curviness, ribbonWidth, vw, vh]);
  ...
  return (
    <div ref={rootRef} className={`text-loop ${className}`.trim()} style={style}>
      <svg
        ...
        viewBox={`0 0 ${vw} ${vh}`}
        preserveAspectRatio={shape === 'line' ? 'xMidYMid slice' : 'xMidYMid meet'}
        role="img"
        aria-label={text}
      >
        <path
